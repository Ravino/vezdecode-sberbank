...
export class UserService {
  ...
  public async create(firstname: string, lastname: string, password: string, divisionId: number, positionId: number, confirmed: boolean, email?: string|null, vkontakteProfileId?: number|null): Promise<any> {
    ...
    let result: any = null;
    try {
      result = await tarantool.sql('insert into users (email, firstname, lastname, display_name, created_at, updated_at, password, division_id, position_id, uuid, confirmed, vkontakte_profile_id, scope) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', params);
    }
    catch(err) {
      console.log(err);
//      throw err;
